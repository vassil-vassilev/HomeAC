...
WebSocketClientAdapter::WebSocketClientAdapter(WebSocketParams& p, size_t bufferSize) :
  ws(),
  params(p),
  isConnected(false)
{
...
void WebSocketClientAdapter::webSocketEvent(WStype_t type, uint8_t * payload, size_t length)
{
  switch(type) {
 ...
 */
int WebSocketClientAdapter::connect(IPAddress ip, uint16_t p)
{
  return connect(0, 0);
}
 ...
 * Returns true if successful, false otherwise
 */
 int WebSocketClientAdapter::connect(const char *h, uint16_t p)
{
...
size_t WebSocketClientAdapter::write(uint8_t b)
{
  if (!connected())
    return 0;
